...
bayiyuan-cli 的调用链路与 wecom-cli / 飞书 CLI 完全相同：IM 机器人 → OpenClaw → exec CLI → HTTP → 后端 API。三者在设计上同构，都是 AI Agent 通过 exec 调用的 CLI 工具。

认证方案的差异不在于调用方式，而在于**后端 API 的归属**：

| | wecom-cli | 飞书 CLI | bayiyuan-cli |
|---|---|---|---|
| 后端 API | 企微 SaaS 平台（不可控） | 飞书 SaaS 平台（不可控） | 自有微服务（可控） |
| 认证体系 | 必须适配平台（Bot ID/Secret → MCP URL） | 必须适配平台（app_id/app_secret → access_token） | 自主决定 |

因为后端是自己的微服务，认证可以怎么简单怎么来，不需要 wecom-cli 那样的扫码 + 加密存储，也不需要飞书 CLI 那样的 OAuth2 客户端凭证流 + token 自动刷新。

关键约束：
- CLI 与 OpenClaw 同机部署，微服务在另一个环境 → 必须走 Gateway
- 环境变量通过 SKILL.md / OpenClaw 配置传入
- 每次 exec 是独立进程，进程间不共享内存

**推荐方案（分两阶段）：**

**Phase 1 (MVP) — 静态服务账号 Token**

```
OpenClaw SKILL.md 配置:
  BAYIYUAN_API_URL=http://gateway-host:port
  BAYIYUAN_API_TOKEN=<后端团队分配的服务账号token>

CLI 内部:
  axios 请求头 → Authorization: Bearer ${process.env.BAYIYUAN_API_TOKEN}
```

- 后端在 Idaas 创建专用服务账号，分配长期有效 token
- 部署者把 token 写进 OpenClaw 环境变量配置
- CLI 仅一行代码读 env var
- 401 时输出明确提示：`[ERROR] 认证失败: Token 已过期或无效，请联系管理员更新 BAYIYUAN_API_TOKEN`

**Phase 2 — OAuth2 client_credentials 自动刷新（按需升级）**

当 Idaas 支持客户端凭证流、且 token 需要自动轮换时启用：

```
OpenClaw 配置:
  BAYIYUAN_OAUTH_CLIENT_ID=cli-service-account
  BAYIYUAN_OAUTH_CLIENT_SECRET=xxx
  BAYIYUAN_OAUTH_TOKEN_URL=http://gateway/oauth2/token

CLI 内部:
  ① 读文件缓存 ~/.config/bayiyuan/token.json (未过期直接用)
  ② 过期 → POST /oauth2/token grant_type=client_credentials
  ③ 写缓存 + 用新 token 发请求
  ④ 401 → 清缓存重试一次
```

文件缓存必要性：每次 exec 是新进程，不能靠内存缓存，文件缓存可跨进程共享避免每次都请求 token。

**待确认事项：**
- 后端能否在 Idaas 分配服务账号 token？有效期多长？
- Idaas 是否支持 OAuth2 client_credentials 模式？
- 直连模式下微服务内网调用是否有认证机制（IP 白名单 / 内部 token）？
...
**已决策：采用方案 A — 直接用 Node.js 做主方案，排除 Java 并行开发。**

理由：

1. **冷启动差距是硬指标** — exec 模式下 Node.js ~200ms vs Java ~1.5s，差距 7.5 倍，并行对比后大概率还是选 Node.js
2. **复杂度远低于现有 CLI** — MVP 仅 7 个 API 端点、3 个命令、静态 Token 认证，对比 wecom-cli（Rust，~1800 行）和 feishu-cli（Go，数百端点），Node.js + axios 完全胜任
3. **并发无瓶颈** — exec 模式每次独立进程，顺序调用 1-2 个 API，Node.js 事件循环绰绰有余
4. **避免迁移债务** — Phase 2 将引入 OAuth client_credentials + Token 文件缓存，集中 Node.js 一条线可避免后续语言迁移重写认证层
